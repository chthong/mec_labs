...
import os
import logging
from oai_rnis.utils import type_util
import requests
import uuid
...
RNIS_DESCRIPTOR = str(os.getenv('RNIS_DESCRIPTOR','../../../etc/rnis-descriptor.yaml'))
MOUNT_CONFIG = str(os.getenv('MOUNT_CONFIG','no')).lower()
RNIS_URL = None ## Only get populated if rnis will be registered to mep automatically
...
def registerRnis(configuration,RNIS_DESCRIPTOR):
    global RNIS_URL
    body = read_yamlfile(RNIS_DESCRIPTOR)
    if 'environment' in configuration['application'].keys() and configuration['application']['environment'] =='testing':
        name = str(uuid.uuid4())
    else:
        name = body['name']
    service = {
    ...
    try: 
        r = requests.post(url=configuration['application']['mep']['registrationUrl'],json=service)
        if r.status_code !=201:
            log.info(f"Not able to register service {body['name']} reason {r.text}")
            rnis_url = ""
        else:
            output = r.json()
            rnis_url = f"http://{output['host']}{output['path']}"
            
    except requests.exceptions.RequestException: 
        log.error("Error while registering OAI-RNIS to OAI-MEP you can register manually")
    return rnis_url

if configuration['application']['mep']['register'] == 'yes':
    RNIS_URL =  registerRnis(configuration,RNIS_DESCRIPTOR)
    log.info(f"Registered OAI-RNIS to OAI-MEP, it is available via mp1 --> {RNIS_URL}")
else:
    log.debug("Not registering OAI-RNIS to OAI-MEP you can register manually")
